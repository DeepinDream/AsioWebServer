...
void PathString(ostream& response, WebRequest& request)
{
...
void start_server(SERVER_TYPE &server) {
	...
	server.resource["^/string/?$"]["POST"] = [](ostream& response, WebRequest& request) {
		// 从 istream 中获取字符串 (*request.content)
		stringstream ss;
		string content("empty param!");
		if(request.content.get() != nullptr){
	...
	server.resource["^/info/?$"]["GET"] = [](ostream& response, WebRequest& request) {
	...
	server.resource["^/match/([0-9a-zA-Z]+)/?$"]["GET"] = [](ostream& response, WebRequest& request) {
	...
	// 将应答 web/ 目录及其子目录中的文件
	// 默认文件: index.html
	server.default_resource["^/?(.*)$"]["GET"] = [](ostream& response, WebRequest& request) {
