...
class Connection
  ...
  public:
    explicit Connection(std::shared_ptr<socket_type> socket,
        ...
        , request_(std::make_shared<WebRequest>())
    {
    }
    ...
    // 应答
    void respond(std::shared_ptr<WebRequest> request)
    {
    ...
    void Handler(Response& res, std::shared_ptr<WebRequest> request)
    {
        // web 响应处理
